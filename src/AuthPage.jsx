...
export default function AuthPage({ onAuth }) {
  ...
  useEffect(() => {
    if (!CLIENT_ID) { setError("VITE_GOOGLE_CLIENT_ID is not set."); return; }
  ...
  useEffect(() => {
    if (ready && btnRef.current) {
      window.google.accounts.id.renderButton(btnRef.current, {
        ...
        text: "signin_with", logo_alignment: "left", width: 320,
      });
  ...
  function handleCredential({ credential }) {
    ...
    const session = { name: user.name, email: user.email, picture: user.picture, sub: user.sub, token: credential };
  ...
  const S = {
    ...
    err:   { background:"rgba(248,113,113,0.08)", border:"1px solid #7f1d1d", borderRadius:8, padding:"10px 14px", color:"#f87171", fontSize:12, marginTop:20 },
